...
function STBS:FrametimeStats(frameTimes)
  ...
  return {average=#values/total,onePercentLow=lowCount/worstTotal,frames=#values,duration=total}
end
...
function STBS:StartFPSBaselineSampling()
  if self.fpsAfterMeasurement or self.fpsAccurateMeasurement or self.fpsBaselineTicker then return end;self.fpsBaselineSamples={}
...
function STBS:CaptureFrameTimes(duration,phase,callback)
  ...
  frame:SetScript("OnUpdate",function(self,elapsed)
    if finitePositive(elapsed) then frameTimes[#frameTimes+1]=elapsed;elapsedTotal=elapsedTotal+elapsed;displayElapsed=displayElapsed+elapsed end
    if displayElapsed>=QUICK_INTERVAL then STBS:NotifyLiveFPS(STBS:ReadFramerate());displayElapsed=0 end
