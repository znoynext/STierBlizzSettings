local _, STBS = ...
local buttonColors={default={0.025,0.07,0.13,0.9,0.9,0.62,0.12},primary={0.02,0.16,0.16,0.95,0.18,0.82,0.62},danger={0.16,0.035,0.04,0.95,0.9,0.24,0.18}}
local function button(parent, text, x, y, width, callback, style)
  ...
  local color=buttonColors[style] or buttonColors.default;b.color=color;b:SetBackdropColor(color[1],color[2],color[3],color[4]);b:SetBackdropBorderColor(color[5],color[6],color[7],0.95)
  ...
  b:SetScript("OnClick", callback);b:SetScript("OnEnter",function(self)self:SetBackdropColor(0.06,0.16,0.28,1);self:SetBackdropBorderColor(1,0.82,0.25,1)end);b:SetScript("OnLeave",function(self)if self.active then self:SetBackdropColor(0.08,0.17,0.3,1) else local c=self.color;self:SetBackdropColor(c[1],c[2],c[3],c[4]) end;local c=self.color;self:SetBackdropBorderColor(c[5],c[6],c[7],0.95)end)
  function b:SetActive(active) self.active=active;local c=self.color;self:SetBackdropColor(active and 0.08 or c[1],active and 0.17 or c[2],active and 0.3 or c[3],active and 1 or c[4]);self.label:SetTextColor(active and 1 or 0.82,active and 0.8 or 0.66,active and 0.18 or 0.12) end
  return b
end
...
local categoryOrder={"graphics","raidGraphics","camera","interface","combat","nameplates","gameplay","controls","chat"}
local function resultSummary(data)
  ...
  for _,category in ipairs(categoryOrder) do local values=(data.categories or {})[category];if values then table.insert(lines, "\n|cff82c5ff"..STBS:L(categoryNames[category] or "OTHER_INTERFACE")..":|r "..STBS:L("CHANGED").." "..(values.changed or 0)..", "..STBS:L("SKIPPED").." "..(values.skipped or 0)..", "..STBS:L("UNAVAILABLE").." "..(values.unavailable or 0)..", "..STBS:L("FAILED").." "..(values.failed or 0)) end end
...
function STBS:CreateUI()
  ...
  local scroll = CreateFrame("ScrollFrame",nil,panel,"UIPanelScrollFrameTemplate"); scroll:SetPoint("TOPLEFT",18,-82); scroll:SetSize(665,292); local content=CreateFrame("Frame",nil,scroll); content:SetWidth(635);content:SetHeight(292);scroll:SetScrollChild(content)
  f.scroll, f.content = scroll, content; f.body=content:CreateFontString(nil,"OVERLAY","GameFontHighlightSmall");f.body:SetPoint("TOPLEFT",8,-6);f.body:SetWidth(610);f.body:SetJustifyH("LEFT");f.body:SetJustifyV("TOP")
  local actionScroll=CreateFrame("ScrollFrame",nil,panel,"UIPanelScrollFrameTemplate");actionScroll:SetPoint("TOPLEFT",18,-390);actionScroll:SetSize(665,138);local actionContent=CreateFrame("Frame",nil,actionScroll);actionContent:SetWidth(635);actionContent:SetHeight(138);actionScroll:SetScrollChild(actionContent);f.actionScroll,f.actionContent=actionScroll,actionContent
  local rule=panel:CreateTexture(nil,"ARTWORK");rule:SetColorTexture(0.35,0.28,0.13,0.8);rule:SetPoint("TOPLEFT",22,-382);rule:SetSize(650,1)
...
function STBS:SetPage(title, text, actions, status)
  ...
  if f.copyBox then f.copyBox:Hide() end;f.scroll:Show()
  f.body:SetText(text or "");f.body:SetWidth(610); f.content:SetHeight(math.max(292, f.body:GetStringHeight()+16)); f.scroll:SetVerticalScroll(0); f.actionScroll:SetVerticalScroll(0); f:Show()
  ...
  local rows=math.ceil(#(actions or {})/2);f.actionContent:SetHeight(math.max(138,rows*40))
  for i,action in ipairs(actions or {}) do table.insert(f.pageButtons,button(f.actionContent,action.label,((i-1)%2)*317,-math.floor((i-1)/2)*40,306,action.fn,action.style)) end
end
function STBS:FormatDiff(plan)
  local lines={"|cff82c5ff"..self:L("DIFF_HEADER").."|r"};local lastCategory
  for _,entry in ipairs(plan or {}) do local category=entry.setting.category;if category~=lastCategory then table.insert(lines,"\n|cffffd100"..self:L(categoryNames[category] or "OTHER_INTERFACE").."|r");lastCategory=category end;local color=entry.status=="changed" and "|cff33ff99" or entry.status=="failed" and "|cffff4040" or "|cff9aa7b8";table.insert(lines,color..self:GetSettingLabel(entry.setting)..":|r "..tostring(entry.current or self:L("UNAVAILABLE")).." → "..entry.value.."  |cff718096("..self:L(string.upper(entry.status))..")|r") end
  return table.concat(lines,"\n")
end
...
function STBS:ShowOfficialPreview(kind)
  ...
  local plan=self:BuildDiff(settings);self:SetPage(self:L("PREVIEW"),self:FormatDiff(plan),{{label=self:L("CONFIRM")..": "..(kind=="all" and self:L("APPLY_ALL") or kind=="graphics" and self:L("APPLY_GRAPHICS") or self:L("APPLY_INTERFACE")),fn=function()STBS:ShowReport(STBS:ApplyOfficial(kind))end,style="primary"},{label=self:L("BACK"),fn=function()STBS:ShowHome()end}},self:L("STATUS")..": "..#plan.." "..self:L("SETTINGS_COUNT"))
end
...
function STBS:OpenSaveDialog(modules)
  if modules.graphics and not self:GetSelectedMode() then self:ShowGraphics();return end
  StaticPopupDialogs["STBS_SAVE"]={text=self:L("PROFILE_NAME"),button1=ACCEPT,button2=CANCEL,hasEditBox=true,maxLetters=self.MAX_PROFILE_NAME_BYTES,OnAccept=function(p)local profile=STBS:SaveCurrent(p.editBox:GetText(),modules);if profile then STBS.selectedProfileId=profile.id end;STBS:ShowProfiles()end,OnShow=function(p)p.editBox:SetText("");p.editBox:SetFocus()end,timeout=0,whileDead=true,hideOnEscape=true,preferredIndex=3};StaticPopup_Show("STBS_SAVE")
end
...
function STBS:ConfirmDeleteProfile(profile)
  StaticPopupDialogs["STBS_DELETE"]={text=self:L("DELETE")..": "..self:SafeText(profile.displayName).."?",button1=ACCEPT,button2=CANCEL,OnAccept=function()STBS:DeleteProfile(profile.id);STBS.selectedProfileId=nil;STBS:ShowProfiles()end,timeout=0,whileDead=true,hideOnEscape=true,preferredIndex=3};StaticPopup_Show("STBS_DELETE")
end
function STBS:ShowProfiles()
  local profiles=self:ListPersonalProfiles();local selected;for _,profile in ipairs(profiles)do if profile.id==self.selectedProfileId then selected=profile;break end end;selected=selected or profiles[1];if selected then self.selectedProfileId=selected.id end
  local lines={"|cff82c5ff"..self:L("PROFILES").."|r"};if #profiles==0 then table.insert(lines,self:L("NO_PROFILES")) else for i,profile in ipairs(profiles)do table.insert(lines,(profile.id==self.selectedProfileId and "|cff33ff99> " or "  ")..i..". "..self:SafeText(profile.displayName).."|r")end end;if selected then table.insert(lines,"\n"..self:L("SELECTED")..": |cffffd100"..self:SafeText(selected.displayName).."|r") end
  ...
  if selected then table.insert(actions,{label=self:L("EXPORT"),fn=function()STBS:ShowExport(selected)end});table.insert(actions,{label=self:L("RENAME"),fn=function()STBS:OpenRenameDialog(selected)end});table.insert(actions,{label=self:L("DELETE"),fn=function()STBS:ConfirmDeleteProfile(selected)end,style="danger"})end
  for _,profile in ipairs(profiles)do local current=profile;table.insert(actions,{label=self:L("SELECT")..": "..self:SafeText(current.displayName),fn=function()STBS.selectedProfileId=current.id;STBS:ShowProfiles()end})end
...
function STBS:ShowBackups()
  ...
  if selected then local hasGraphics=self:BackupHasModule(selected,"graphics");local hasInterface=self:BackupHasModule(selected,"interfaceGameplay");if hasGraphics and hasInterface then table.insert(actions,{label=self:L("RESTORE_ALL"),fn=function()STBS:ShowReport(STBS:RestoreBackup(STBS.selectedBackupIndex,{graphics=true,interfaceGameplay=true}))end,style="primary"})end;if hasGraphics then table.insert(actions,{label=self:L("RESTORE_GRAPHICS"),fn=function()STBS:ShowReport(STBS:RestoreBackup(STBS.selectedBackupIndex,{graphics=true}))end})end;if hasInterface then table.insert(actions,{label=self:L("RESTORE_INTERFACE"),fn=function()STBS:ShowReport(STBS:RestoreBackup(STBS.selectedBackupIndex,{interfaceGameplay=true}))end})end end
  for i=1,#backups do local index=i;table.insert(actions,{label=self:L("SELECT")..": #"..index,fn=function()STBS.selectedBackupIndex=index;STBS:ShowBackups()end})end;self:SetPage(self:L("BACKUPS"),table.concat(lines,"\n"),actions)
end
...
function STBS:ShowCopyBox(text)
  self:CreateUI();local f=self.ui;local box=f.copyBox;if not box then box=CreateFrame("EditBox",nil,f.content,"InputBoxTemplate");box:SetMultiLine(true);box:SetAutoFocus(false);box:SetFontObject(ChatFontNormal);box:SetWidth(600);box:SetHeight(275);box:SetPoint("TOPLEFT",8,-6);box:SetScript("OnEscapePressed",function(x)x:ClearFocus()end);f.copyBox=box end;f.body:SetText("");box:SetText(text);box:Show();box:SetFocus();box:HighlightText();f:Show()
end
function STBS:ShowExport(profile) local modules=profile.capturedModules or {graphics=true,interfaceGameplay=true};local output=self:ExportProfile(profile,modules);self:SetPage(self:L("EXPORT"),self:L("EXPORT_HELP").."\n\n"..self:L("SELECTED")..": "..self:SafeText(profile.displayName).."\n"..self:L("SIZE")..": "..#output.." bytes",{{label=self:L("COPY"),fn=function()STBS:ShowCopyBox(output)end},{label=self:L("BACK"),fn=function()STBS:ShowProfiles()end}}) end
