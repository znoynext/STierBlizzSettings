...
- Default change previews report concise counts and outcomes. Optional **Show changes** opens a human-readable advanced view with localized labels, current/target values, short explanations and qualitative primary-effect metadata; matching entries stay hidden, unavailable/skipped entries remain separate, and raw CVar names are not normal-user primary text. Impact levels describe the setting's main product effect, not a promised FPS delta.
