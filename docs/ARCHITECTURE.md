...
`Core` owns startup, compatibility, SavedVariables, FPS sampling and diagnostics. `Settings` contains the curated registry, validation, verified read/write operations, diffs, backups and transactions. `Profiles` owns schema, migration, built-ins and personal profiles. `ImportExport` implements deterministic serialization and a data-only parser; it never calls `loadstring`. `UI` is lazily created from Blizzard frame primitives and exposes Graphics, Profiles and About.
...
The Graphics tab owns the official mode selector, real in-game example, concise outcome preview, confirmation, live FPS, automatic local FPS comparison, post-apply Reload UI action and latest-backup undo. FPS sampling uses Blizzard's documented `GetFramerate()` function at 0.5-second intervals. Results are character-local estimates and are never presented as guaranteed gains.
