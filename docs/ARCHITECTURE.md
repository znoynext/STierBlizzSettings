...
- A backup is still created when the completed plan/snapshot contains no actual writes, so identical, skipped, or unavailable-only operations can create no-op history entries.
