...
Backup triggers remain free-form strings rather than structured provenance. Stable identity solves target races but does not yet classify user, automatic, safety, or temporary records as typed provenance.
...
The account schema constant is currently 4. `MigrateDatabase` owns an ordered table of migrations keyed by source schema. It preflights that every required `N -> N+1` function exists, runs each step in order, and advances `schemaVersion` only after that step succeeds. Migration `2 -> 3` moves legacy preset/mode intent into a runtime draft, initializes persisted applied preset as Custom, and sets a one-time flag so `ADDON_LOADED` can inspect real graphics CVars before committing applied preset/mode. Migration `3 -> 4` preserves valid unique backup IDs, assigns sequence-backed IDs to legacy records without one, resolves collisions, and persists `backupSequence`. Repeated initialization at schema 4 normalizes but never reruns either migration or changes retained IDs.
...
Future persisted changes add exactly one migration under their source version. Stable backup identity belongs to schema 4; structured provenance remains a separate future schema concern. Device-local preferences must be preserved by shared-data import; the current full-addon import preserves window/widget fields but omits `minimapAngle` when replacing preferences.
