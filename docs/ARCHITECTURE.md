...
4. Unless `skipBackup` is set, capture a snapshot of all readable values in the selected modules and insert it into backup history with the workflow's normalized `backupSource` while retaining its diagnostic trigger.
