...
- `Settings/Diff.lua` is the single source for the five-field summary shape and status accounting. Built-in/personal/FPS-candidate Graphics, Zone Graphics, UI Tweaks, profile/full-addon import, transaction results, pending feedback, and Backup Restore consume compatible summaries; previews show skipped, unavailable, and failed as separate counts.
