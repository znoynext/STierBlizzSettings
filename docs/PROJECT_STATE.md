...
- Outside combat it builds a diff, then re-reads every selected changed entry into an immutable transaction snapshot containing its exact previous value and target. A missing or invalid previous value returns `snapshot-failed` before backup creation and before the first write.
