...
- Built-in presets are **PRO**, **Optimized**, and **Quality**. The header detects the actually active preset and otherwise shows Custom. Detection is cached across ordinary page renders; verified Graphics apply/no-op and uncertain rollback failure invalidate it, while successful addon mutation workflows refresh on their normal UI result path. Opening the window and applied-state sync force a live scan so external Blizzard Settings or migration-driven changes are detected without permanent polling.
...
- Standalone and both preset-comparison phases share one lazily created frame-time sampler frame. Its active phase, duration, elapsed time, frame-time buffer and callback are reset between sessions; overlap is rejected, Cancel clears ownership, and a generation guard prevents an old update handler from affecting a restarted capture.
