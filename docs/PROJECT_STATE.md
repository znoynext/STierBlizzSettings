...
- Core DB schema `3` uses an ordered one-step migration pipeline. The known schema `2 -> 3` migration preserves old preset/mode as the runtime draft, marks persisted applied state as Custom, and requests one actual-CVar sync after addon load. Fresh, missing, or corrupted version metadata uses a separate unversioned recovery path rather than invented historical migrations. A future schema is never normalized or rewritten: the original SavedVariables stay untouched, transactions fail closed, and runtime consumers receive an isolated safe view. Profile migration remains separate and currently only normalizes legacy schema-1 data.
