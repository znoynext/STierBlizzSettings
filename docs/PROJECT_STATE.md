...
- Full-addon import applies graphics and UI Tweaks in one backup-first transaction, patches only the validated shared preference contract (benchmark mode, performance-widget enabled state, and Zone Graphics), and replaces personal profiles. The existing preferences table is retained, so backup limit, window/minimap/widget layout, and unknown future device-local fields survive. Import is rejected in combat rather than queued.
