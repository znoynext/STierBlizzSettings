...
- Restore and delete resolve the selected record by stable ID rather than list position. Restore filters removed registry keys; a real restore creates an ID-addressed safety backup before transactional apply, while an already-matching target returns `unchanged` without redundant safety history.
