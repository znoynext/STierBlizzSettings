...
Every registry entry also carries presentation metadata for the optional detailed-diff view: a Blizzard-localized or addon-localized user-facing label plus a short localized explanation. This metadata is descriptive only. Raw CVar keys remain available internally for validated identity and diagnostics but are not used as primary normal-user text.
