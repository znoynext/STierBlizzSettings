...
- Separate Profiles, Backups and Import / Export views. Backup restore/delete actions stay visible, and one `STBSA1` string can transfer current graphics, available UI Tweaks, verified applied preset/mode, stored benchmark choice, performance-widget enabled state, zone rules and personal profiles through large export and import fields. Backup history, runtime selection and screen positions are not exported; importing preserves the target installation's window, widget and minimap layout plus other local preferences.
