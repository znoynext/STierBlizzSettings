...
- Concise result preview without a technical CVar list, with separate changed/already-matching/skipped/unavailable/failed counts and an explicit confirmation before applying. An optional **Show changes** action expands only relevant items into localized names, current/target values, short explanations and a qualitative primary-effect level; matching settings stay hidden.
- A larger, resizable native WoW window that remembers its valid size and header-dragged position. If the window or FPS/ping widget becomes unreachable, `/stier reset` or **Reset Window Position** in Blizzard Settings restores both positions without touching profiles, backups or gameplay settings; a valid window size is preserved.
- Separate Profiles, Backups and Import / Export views. Backup restore/delete actions stay visible, and one `STBSA1` string can transfer current graphics, available UI Tweaks, verified applied preset/mode, performance-widget enabled state, zone rules and personal profiles through large export and import fields. Backup history, runtime selection and screen positions are not exported; importing preserves the target installation's window, widget and minimap layout plus other local preferences. Old bundles with the removed benchmark-mode field remain compatible and safely ignore it.
- Transactional writes with automatic backup and read-back verification for real changes. Already-active settings return `unchanged` without adding backup or transaction history.
- Backup restore clearly distinguishes queued, complete, partial and unavailable results. Compatible legacy settings still restore safely when another saved CVar was removed or is unavailable, and failed recovery records are retained even at the smallest history limit instead of being trimmed before retry.
- Graphics selection is a draft until Apply succeeds. Combat-delayed Graphics, Zone Graphics, UI Tweaks and recovery share one verified completion workflow, so failed, cancelled or rolled-back work cannot claim applied preset/mode.
- One-click **Undo graphics changes** targets the latest retained user-initiated graphics change and skips automatic Zone Graphics, temporary FPS comparison, and restore-safety records.
...
- Personal graphics profiles plus a separate backup-history view in the Profiles section. Save Graphics reads Blizzard's actual Raid/Battleground toggle instead of trusting an older saved mode and never writes CVars. Older split profiles are clearly marked: conversion saves a new unified copy without changing current graphics or deleting the original, while exact split application remains an advanced backup-first action.
