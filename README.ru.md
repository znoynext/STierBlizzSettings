...
- Краткий предпросмотр результата без технического списка CVar, с отдельными счётчиками изменений, совпадений, пропущенных, недоступных и ошибочных настроек, и отдельное подтверждение перед применением. Необязательная кнопка **«Показать изменения»** раскрывает только важные пункты: понятное название, текущее и целевое значение, краткое объяснение и качественный уровень основного эффекта; совпадающие настройки скрыты.
- Увеличенное масштабируемое окно запоминает корректный размер и положение после перемещения за шапку. Если окно или виджет FPS/пинга стали недоступны, `/stier reset` либо **«Сбросить положение окна»** в настройках Blizzard возвращает обе позиции, не затрагивая профили, бэкапы и игровые настройки; корректный размер окна сохраняется.
...
- Применение, переименование, экспорт и удаление профилей; восстановление и удаление бэкапов. «Сохранить графику» читает фактическое состояние переключателя Blizzard для рейдов и полей боя, а не доверяет старому сохранённому режиму, и никогда не записывает CVar. Старые раздельные профили явно помечаются: преобразование сохраняет новую единую копию, не меняя текущую графику и не удаляя исходник, а точное применение старого split-режима остаётся расширенным действием с бэкапом.
